...
proc scanAppDir(
  ...
  if target.isDir:
    ...
    if not safeFileExists(fullPath):
      return
    let sev = if isWorldReadable(fullPath): svCritical
              elif isGroupReadable(fullPath): svHigh
              else: svMedium
    result.findings.add(makeFinding(
      fullPath,
      target.description,
...
proc scanDbCredFiles(config: HarvestConfig, result: var CollectorResult) =
  ...
  if safeFileExists(mycnfPath):
    ...
    let sev = if isWorldReadable(mycnfPath): svCritical
              elif hasPassword: svHigh
              else: svMedium
  ...
  if safeFileExists(mongoPath):
    ...
    let hasCreds = "password" in content.toLowerAscii() or
                   "auth" in content.toLowerAscii()
    let sev = if isWorldReadable(mongoPath): svCritical
              elif hasCreds: svHigh
              else: svMedium
...
proc scanDevTokenFiles(config: HarvestConfig, result: var CollectorResult) =
  ...
  if safeFileExists(npmrcPath):
    ...
    let sev = if isWorldReadable(npmrcPath): svCritical
              elif hasToken: svHigh
              else: svInfo
  ...
  if safeFileExists(pypircPath):
    ...
    let hasPassword = "password" in content.toLowerAscii()
    let sev = if isWorldReadable(pypircPath): svCritical
              elif hasPassword: svHigh
              else: svMedium
  ...
  if safeFileExists(ghPath):
    ...
    let sev = if isWorldReadable(ghPath): svCritical
              elif hasOauth: svHigh
              else: svMedium
...
proc scanInfraTokenFiles(config: HarvestConfig, result: var CollectorResult) =
  ...
  if safeFileExists(tfPath):
    ...
    let sev = if isWorldReadable(tfPath): svCritical
              elif hasToken: svHigh
              else: svMedium
  ...
  if safeFileExists(helmPath):
    ...
    let hasPassword = "password" in content.toLowerAscii()
    let sev = if isWorldReadable(helmPath): svCritical
              elif hasPassword: svHigh
              else: svInfo
  ...
  if safeFileExists(rclonePath):
    ...
    let hasCreds = "pass" in content.toLowerAscii() or
                   "token" in content.toLowerAscii() or
                   "key" in content.toLowerAscii()
    let sev = if isWorldReadable(rclonePath): svCritical
              elif hasCreds: svHigh
              else: svMedium
...
proc scanDockerConfig(config: HarvestConfig, result: var CollectorResult) =
  ...
  let sev = if isWorldReadable(dockerPath): svCritical
            elif hasAuth: svHigh
            else: svMedium
...
proc collect*(config: HarvestConfig): CollectorResult =
  ...
  let appTargets = [
    ...
    AppTarget(path: VsCodeDir, name: "VS Code",
              description: "VS Code configuration directory", isDir: true),
    AppTarget(path: VsCodeUserSettings, name: "VS Code Settings",
              description: "VS Code user settings (may contain tokens)", isDir: false)
  ]
